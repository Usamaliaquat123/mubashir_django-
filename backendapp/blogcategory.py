...
def category_list(request, template_name='backend/blog/category/list.html'):
    try:
        #get post
        if request.method == 'POST':
            if 'clearbtn' in request.POST:
                #clear form
                if 'blog-category-search-post' in request.session:
                    del request.session['blog-category-search-post']
                return redirect(str(settings.SITE_URL)+'/backend/blog/category/listing')
            else:
                request.session['blog-category-search-post']     = request.POST
                Post_Data                               = request.POST
        else:
            if 'blog-category-search-post' in request.session:
                Post_Data = request.session['blog-category-search-post']
            else:
                Post_Data = []

        #check session
        if Post_Data:
            form = BlogCategoryListFilter(Post_Data)
        else:
            form = BlogCategoryListFilter()
        ...
        objs        = BlogCategory.objects.filter(complexQuery).order_by('name')
...
def category_status(request):
    try:
        ...
        if request.method == 'POST':
            ...
            records = BlogCategory.objects.filter(id__in = ids)
            if action == 'active':
                status = True
...
@check_role_permission() # - check role permission
@cache_control(no_cache=True, must_revalidate=True, no_store=True)
def category_create(request, template_name='backend/blog/category/add.html'):
    try:
        if request.method == 'POST':
...
@check_role_permission() # - check role permission
@cache_control(no_cache=True, must_revalidate=True, no_store=True)
def category_update(request, pk, template_name='backend/blog/category/edit.html'):
    try:
...
@check_role_permission() # - check role permission
@cache_control(no_cache=True, must_revalidate=True, no_store=True)
def category_delete(request, pk, template_name='backend/blog/category/confirm_delete.html'):
    try:
        category = get_object_or_404(BlogCategory, pk=pk)
        if request.method=='POST':
            category.delete()
